...
bool gdwg::Graph<N, E>::InsertEdge(const N &src, const N &dst, const E &w) {
  ...
  bool foundPos = false;
  for (auto it = srcNode->outGoing_.begin(); it != srcNode->outGoing_.end();
       ++it) {
    if (*newEdge < **it) {
  ...
  foundPos = false;
  for (auto it = dstNode->inGoing_.begin(); it != dstNode->inGoing_.end();
       ++it) {
    if (*newEdge < **it) {
...
template <typename N, typename E> void gdwg::Graph<N, E>::Clear() {
  nodes_.clear();
}
...
template <typename N, typename E>
bool gdwg::Graph<N, E>::IsConnectedWeight(const N &src, const N &dst,
                                          const E &weight) {
...
gdwg::Graph<N, E>::erase(const_iterator it) {
  ...
  for (auto iter = dst->outGoing_.begin(); iter != dst->outGoing_.end();
       ++iter) {
    if (*iter == edge) {
...
typename gdwg::Graph<N, E>::const_iterator gdwg::Graph<N, E>::cbegin() {
  ...
  while (itBeg != itEnd &&
         itBeg->second->outGoing_.cbegin() == itBeg->second->outGoing_.cend()) {
    itBeg++;
  }
  if (itBeg != itEnd) {
...
gdwg::Graph<N, E>::crbegin() {
  ...
  while (itBeg != itEnd && itBeg->second->outGoing_.crbegin() ==
                               itBeg->second->outGoing_.crend()) {
    itBeg++;
  }
  if (itBeg != itEnd) {
...
typename gdwg::Graph<N, E>::const_reverse_iterator gdwg::Graph<N, E>::crend() {
  typename std::vector<std::shared_ptr<Edge>>::const_reverse_iterator itEdge;
  return const_reverse_iterator(nodes_.crend(), nodes_.crbegin(),
                                nodes_.crend(), itEdge);
}
